...
#include "../jsmn.h"
#include "serial_module.h"   // per serial_printf
#include <string.h>          // serve per strlen e strncmp

#define MAX_TOKENS 64   // numero massimo di token gestibili (configurabile)
...
static int vtokeq(const char *s, jsmntok_t *t, unsigned long numtok,
  ...
  if (numtok > 0) {
    ...
    for (i = 0; i < numtok; i++) {
      ...
      if (t[i].type != type) {
        serial_printf("token %lu type is %d, not %d\n", i, t[i].type, type);
        return 0;
      }
      if (start != -1 && end != -1) {
        if (t[i].start != start) {
          serial_printf("token %lu start is %d, not %d\n", i, t[i].start, start);
          return 0;
        }
        if (t[i].end != end) {
          serial_printf("token %lu end is %d, not %d\n", i, t[i].end, end);
          return 0;
        }
      }
      if (size != -1 && t[i].size != size) {
        serial_printf("token %lu size is %d, not %d\n", i, t[i].size, size);
        return 0;
      }
      ...
      if (s != NULL && value != NULL) {
        ...
        if (strlen(value) != (unsigned long)(t[i].end - t[i].start) ||
          ...
          serial_printf("token %lu value is %.*s, not %s\n",
...
static int parse(const char *s, int status, unsigned long numtok, ...) {
  ...
  va_list args;
  jsmn_parser p;
  jsmntok_t t[MAX_TOKENS];   // allocazione statica

  if (numtok > MAX_TOKENS) {
    serial_printf("Errore: numtok %lu > MAX_TOKENS %d\n", numtok, MAX_TOKENS);
    return 0;
  }
  ...
  if (r != status) {
    serial_printf("status is %d, not %d\n", r, status);
    return 0;
  }
